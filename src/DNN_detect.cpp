...
//
#include "../include/DNN_detect.h"

int DNN_detect::dnn_detect(Mat frame) {
    return net_forward(img_processing(std::move(frame), IS_GRAY), read_net(NET_PATH));
}

int DNN_detect::dnn_detect(const String& imgpath) {
    Mat frame = imread(imgpath);
    return net_forward(img_processing(frame, IS_GRAY), read_net(NET_PATH));
}
...
Mat DNN_detect::img_processing(Mat ori_img, bool is_gray) {
    Mat out_blob;
    if(is_gray) {cvtColor(ori_img, ori_img, COLOR_BGR2GRAY);}
